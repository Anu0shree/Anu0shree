# 👋 Hi, I'm Anushree Venkatraju

💻 **Cloud & DevOps Engineer | AWS Certified Cloud Practitioner | Java Developer**

---

## 🚀 About Me
- 4+ years of experience building enterprise healthcare applications in Java.
- Transitioning into **Cloud & DevOps Engineering** with hands‑on AWS, Terraform, Ansible, Docker, and CI/CD pipelines.
- Currently working on a **Cloud DevOps Dashboard** project on AWS (Spring Boot + IaC + Monitoring).
- Preparing for **AWS Solutions Architect – Associate** certification.
- Exploring **Kubernetes** and real‑world production DevOps practices.
- Fun fact: I taught myself DevOps by breaking my own AWS deployment — and then fixing it!

---

## 🌟 Highlights
- ✅ 4+ years Java enterprise application support & release management  
- ✅ AWS Certified Cloud Practitioner (2026–2029)  
- ✅ Hands‑on with **Terraform, Ansible, Docker, Jenkins, GitHub Actions**  
- ✅ SQL optimization: improved query performance by 40–60%  
- ✅ Healthcare domain experience (Revenue Cycle Management)  

---

## 🛠️ Tech Stack
![Java](https://img.shields.io/badge/Java-Enterprise-blue)
![Spring Boot](https://img.shields.io/badge/SpringBoot-Microservices-green)
![AWS](https://img.shields.io/badge/AWS-Cloud-orange)
![Terraform](https://img.shields.io/badge/Terraform-IaC-blue)
![Ansible](https://img.shields.io/badge/Ansible-Automation-red)
![Docker](https://img.shields.io/badge/Docker-Containers-lightblue)
![Jenkins](https://img.shields.io/badge/Jenkins-CI/CD-yellow)
![GitHub Actions](https://img.shields.io/badge/GitHub-Actions-black)


---

# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=Anu0shree&theme=dark&hide_border=false&include_all_commits=false&count_private=false)<br/>
![](https://streak-stats.demolab.com/?user=Anu0shree&theme=dark&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=Anu0shree&theme=dark&hide_border=false&include_all_commits=false&count_private=false&layout=compact)

---

## 📂 Featured Project

### 🔹 [Cloud DevOps Dashboard](https://github.com/Anu0shree/cloud-devops-dashboard)

End-to-end DevOps project using Spring Boot, AWS, Terraform, Ansible, Docker, GitHub Actions, Prometheus, and Grafana.

- Provisioned AWS infrastructure using Terraform.
- Configured EC2 using Ansible.
- Containerized and deployed the application using Docker.
- Implemented GitHub Actions CI/CD with Docker Hub.
- Added Actuator health and Prometheus metrics.
- Visualized application and infrastructure metrics using Grafana.
- Implemented Git-SHA image tagging and deployment concurrency controls to prevent stale deployments. 

---

## 🤝 Connect With Me
- [LinkedIn](https://linkedin.com/in/anushree-venkatraju)  
- [Email](mailto:anu0shree@gmail.com)  
